...
vector<string> baseCmds = {"clear","pwd","ping ","whoami","echo ","ifconfig","cls","dir","ls","bash","cd","sudo su"};
...
void setupConsoleBuffer() {
    ...
    COORD newSize;
    newSize.X = cs.dwSize.X;
    newSize.Y = 9999999;   // عدد الأسطر (كبرها براحتك)
...
bool isCdCommand(const std::string& input) {

    if (input.rfind("cd ", 0) == 0) return true;
    return false;
}
...
void updateGhostCdOnly() {
    ghost.clear();

    if (!isCdCommand(buffer))
        return; // ❌ لا ghost إن لم يكن cd

    std::string partial;
    size_t pos = buffer.find(' ');
    if (pos != std::string::npos)
        partial = buffer.substr(pos + 1);
    ...
    do {
        if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            if (name.rfind(partial, 0) == 0) {
                ghost = name.substr(partial.size());
                break; // أول تطابق فقط (مثل ghost الحالي)
            }
        }
        ...
        while (true) {
            ...
            if (ch == 224) {
                ...
                if (ch == 72) { 
                    ...
                    if(isCdCommand(buffer)){updateGhostCdOnly();}else{updateGhost();}
            ...
            if (ch == 8) {
                ...
                if(isCdCommand(buffer)){updateGhostCdOnly();}else{updateGhost();} redrawLine(); continue;
            }
            ...
            if (isprint(ch)) {
                ...
                cursorPos++;
                if(isCdCommand(buffer)){updateGhostCdOnly();}else{updateGhost();} redrawLine();
            }
        }
